#!/usr/bin/env bash
# fm-robot host installer (macOS only). Brings a Mac to the point it can launch the
# baked robot image: sets up the container runtime (delegated to fm-docker), then
# pulls the published fm-robot image locally. Idempotent — safe to re-run.
#
# Install gets it running; clone is the dev path. This does not clone a workspace
# — for the edit-rebuild loop, clone fm-robot and use run.sh from the checkout.
#
...
#   curl -fsSL https://raw.githubusercontent.com/first-motive/fm-robot/main/install.sh | bash
#
# Inspect before running (always offer this path):
#   curl -fsSL https://raw.githubusercontent.com/first-motive/fm-robot/main/install.sh -o install.sh
...
# the pulled fm-robot image (the container runtime is left in place). The body is
# wrapped in main() and called on the last line, so a truncated curl|bash leaves
# an incomplete function that never runs.
set -euo pipefail

IMAGE="ghcr.io/first-motive/fm-robot:humble"
...
usage() {
  cat <<'EOF'
install.sh — set up the fm-robot host (macOS only)

Usage: ./install.sh [install|uninstall] [options]

  install      set up the container runtime, pull the fm-robot image (default)
  uninstall    remove the pulled fm-robot image (runtime is left in place)
...
# fm-docker base image; this script pulls the fm-robot image below instead.
setup_runtime() {
...
do_install() {
  ...
  echo "Done. Launch the robot view: curl -fsSL $FM_ROBOT_RAW/run.sh | bash"
}

do_uninstall() {
  local dry="$1"
  echo "fm-robot uninstall (macOS) ..."
  if [ "$dry" = "1" ]; then
    echo "  would remove image $IMAGE (if present); container runtime left in place"
    return 0
  fi
  # Remove only what this installer owns: the pulled fm-robot image. The container
